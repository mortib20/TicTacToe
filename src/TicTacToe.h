...
private:
	...
	static const int ColumnLength = 3;
	...
	SDL_Window* Window = SDL_CreateWindow("TicTacToe", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, Width, Height + 1, 0);
	...
	int GetRow() {
		...
		for (int r = 0; r < RowLength; r++)
			if (MouseY >= RowSize * r && MouseY <= RowSize * (r + 1))
				return r;
	}
	...
	void CheckWinner(int player)
	{
		...
		for (auto c = 0; c < ColumnLength; c++)
			for (auto r = 0; r < RowLength; r++)
				if (GameField[c][r] == player && GameField[c + 1][r] == player && GameField[c + 2][r] == player)
					Winner = true;

		// Column Check
		for (auto c = 0; c < ColumnLength; c++)
			for (auto r = 0; r < 1; r++)
				if (GameField[c][r] == player && GameField[c][r + 1] == player && GameField[c][r + 2] == player)
					Winner = true;
